<template>
  <div class="panel flex flex-col h-full">
    <div class="panel-title">
      SISTEMAS ORION
      <span class="ml-auto mono text-[9px] text-slate-600">ESM · LSS · TM</span>
    </div>
    <div class="flex-1 p-3 flex flex-col gap-2.5">

      <!-- System bars -->
      <div v-for="sys in systems" :key="sys.id" class="flex items-center gap-2">
        <div class="mono text-xs w-20 flex-shrink-0 uppercase tracking-wide font-semibold" style="color:#9ecfe0">{{ sys.label }}</div>
        <div class="flex-1 h-2 bg-slate-800/50 rounded-sm overflow-hidden">
      ...
      <!-- Gauges -->
      <div class="border-t border-white/5 pt-3 flex gap-3 justify-around mt-auto">
        <div v-for="g in gauges" :key="g.id" class="flex flex-col items-center gap-1">
          <svg width="62" height="62" viewBox="0 0 62 62">
            ...
            <text x="31" y="38" text-anchor="middle" font-size="7"
              font-family="'Share Tech Mono',monospace" fill="rgba(255,255,255,0.3)">%</text>
          </svg>
          <div class="text-[10px] mono uppercase tracking-widest font-semibold" style="color:#7cb9d4">{{ g.label }}</div>
...
const systems = [
  { id: 'prop',  label: 'PROP/ESM',  pct: 91,  barColor: '#a78bfa', dotColor: '#00ff88' },
  { id: 'elec',  label: 'ELEC/PWR',  pct: 88,  barColor: '#a78bfa', dotColor: '#00ff88' },
  { id: 'life',  label: 'LIFE SUP',  pct: 97,  barColor: '#a78bfa', dotColor: '#00ff88' },
  { id: 'comms', label: 'COMM/DSN',  pct: 100, barColor: '#a78bfa', dotColor: '#00ff88' },
  { id: 'therm', label: 'THERMAL',   pct: 94,  barColor: '#a78bfa', dotColor: '#00ff88' },
  { id: 'toilet',label: 'TOILET',    pct: 72,  barColor: '#f59e0b', dotColor: '#f59e0b' },
]

const gauges = [
  { id: 'o2',  label: 'O₂',   pct: 98, color: '#a78bfa' },
  { id: 'h2o', label: 'H₂O',  pct: 87, color: '#c4b5fd' },
  { id: 'bat', label: 'BATT', pct: 84, color: '#00ff88' },
]
</script>
